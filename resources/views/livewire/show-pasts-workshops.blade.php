<div>
    ...
    <table class="w-full mx-auto" wire:loading.remove>
      ...
      <tr class = 'border-b border-gray-200'>
        <td class="p-3 ">{{ \Illuminate\Support\Str::limit($pastsWorkshop->name, 45, $end='...') }}</td>
        @php 
           $limited_participants = true;
           if (!$pastsWorkshop->limited_participants)   $limited_participants = false;
        @endphp
        <td class = "w-32" ><div class="w-6  flex justify-center items-start rounded text-white text-xs  @if($limited_participants) bg-orange-500 @else bg-green-500 @endif ">@if($limited_participants) {{$pastsWorkshop->limited_participants}} @else no @endif</div></td>

        <td ><a href="#" class = "text-blue-600"> {{\Carbon\Carbon::parse($pastsWorkshop->time)->format('d F Y h:m') }}</a></td>
        <td><img class="object-cover rounded" alt="hero" src="{{$pastsWorkshop->img_workshop ? asset('/storage/' . $pastsWorkshop->img_workshop) : asset('/img/test.jpg')}}" width="50"></td>
        <td class = "flex items-center " >
          
           <a href="/workshopManage/{{$pastsWorkshop->id}}/{{$pastsWorkshop->limited_participants ? $pastsWorkshop->limited_participants : 'null'}}/edit" class="bg-sky-500 text-white px-3 p-2  text-xs rounded mr-3 my-2 hover:bg-sky-600">
          ...
          <a href={{ route('adminsuperadmin.showParticipants',$pastsWorkshop->id)}} class="w-28 bg-sky-600 text-white p-2 pr-0 text-xs rounded flex items-center  my-2 hover:bg-sky-700">
            ...
            <a href={{ route('adminsuperadmin.showPDF',$pastsWorkshop->id)}}><i class="fa-solid fa-file-arrow-down pl-1"></i></a>
              ...
              <a class="w-28 w-1/2 bg-sky-600 text-white p-2 pr-0 text-xs rounded flex items-center  my-2 hover:bg-sky-700" href={{ route('adminsuperadmin.showUser',$pastsWorkshop->id) }}>
                <h2 class = "mx-1 ">Add Participant</h2>   
              </a>
            </td>
            </tr>
      @endforeach
