<div class="lg:w-full w-[1000px] py-4">
  ...
  <table class="w-full mx-auto" wire:loading.remove>
    ...
    <tr class='border-t border-gray-200' style="border-top-width: 0.01em">
      ...
      <td class="w-32">
        <div
          class="w-16 px-2 py-1 bg-gradient-to-r @if($upcomingWorkshop->limited_participants) from-orange-300 to-orange-500 @else from-lime-500 to-green-500 @endif  flex justify-center items-center rounded-lg text-white text-xs font-medium">
          @if($upcomingWorkshop->limited_participants) {{$upcomingWorkshop->limited_participants}} @else unlimited
          @endif
        </div>
      </td>

      <td class="text-xs text-gray-500 ">
        {{\Carbon\Carbon::parse($upcomingWorkshop->time)->format('d F Y h:m')}}
      </td>
      <td>
        <div class="relative  flex items-center" x-data="{ open: false }" style = "display:relative;">
          ...
          <ul  class="bg-white mt-2 z-10 shadow-lg border border-gray-100 rounded-lg w-40 py-1 "
            ...
            <li>
                <button wire:click = 'deleteWorkshop({{$upcomingWorkshop->id}})' @click="open = !open" class="w-full text-left text-red-400 py-1 px-3 hover:bg-indigo-100 border-b">
                  <i class="fa-solid fa-trash-can   fa-sm"></i> Delete
                </button>
            </li>
            <li>
              <a class="py-1 px-3 block hover:bg-indigo-100 text-blue-900 border-b" href={{
               route('adminsuperadmin.showPDF',$upcomingWorkshop->id)}}>
            ...
            <li class = "flex items-center px-3 py-1 border-b hover:bg-indigo-100">
              <i class="fa-regular fa-calendar-plus mr-1.5 text-green-300"></i>
                <input wire:change = 'startWorkshop({{$upcomingWorkshop->id}})' type ="checkbox" @click="open = !open"  @if($upcomingWorkshop->workshop_startTime != null) checked  @endif class = "mr-2 rounded"/>
                Started
            </li>
            <li>
              <a class="py-1 px-3 block hover:bg-indigo-100 " href={{
                route('adminsuperadmin.showUser',$upcomingWorkshop->id) }}>
                <i class="fa-solid fa-user-plus text-gray-400 fa-sm"></i> Add Participant
              </a>
            </li>
            <li>
              <a class="py-1 px-3 block hover:bg-indigo-100 " href="/workshops/manage/streaminglive/{{$upcomingWorkshop->id}}">
      ...
      <td>
        <a href={{ route('adminsuperadmin.showParticipants',$upcomingWorkshop)}} class="w-fit px-3 py-2
