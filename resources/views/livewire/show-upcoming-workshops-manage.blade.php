<div class="w-full bg-white shadow-md  rounded-xl py-4 ">
  ...
  <table class="w-full mx-auto" wire:loading.remove>
    ...
    <tr class='border-t border-gray-200' style="border-top-width: 0.01em">
      ...
      <td class="w-32">
        <div class="w-16 px-2 py-1 bg-gradient-to-r @if($upcomingWorkshop->limited_participants) from-orange-300 to-orange-500 @else from-lime-500 to-green-500 @endif  flex justify-center items-center rounded-lg text-white text-xs font-medium">
      ...
      <td>
        <div class=" relative flex items-center " x-data="{ open: false }">
          ...
          <ul class="bg-white absolute top-0 mt-2 z-10 shadow-lg border border-gray-100 rounded-lg w-40 py-1 " x-show="open" @click.outside="open = false">
            <li>
              <p class="text-xs pl-3 p-2 text-gray-400 ">Manage Workshop</p>
            </li>
            <li><a href="/workshops/manage/{{$upcomingWorkshop->id}}/{{$upcomingWorkshop->limited_participants ? $upcomingWorkshop->limited_participants : 'null'}}/edit" class="py-1 px-3 border-b block hover:bg-indigo-100 ">
            ...
            <li>
              <a class="w-full text-left py-1 px-3 block hover:bg-indigo-100 border-b text-blue-900 " href={{ route('adminsuperadmin.showPDF',$upcomingWorkshop->id)}}>
